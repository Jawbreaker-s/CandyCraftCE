...
open class AxisSet private constructor(protected var ax: Int = 0b000) : Set<Axis> {
    ...
    fun immutable(): AxisSet {
        return if (this is Mutable) AxisSet(ax) else this
    }
    ...
    infix fun or(another: AxisSet): AxisSet {
        val nax = ax or another.ax
        return AxisSet(nax)
    }

    operator fun not(): AxisSet {
    ...
    override fun hashCode(): Int = ax.hashCode()
    override val size: Int get() = Integer.bitCount(ax)
    override fun isEmpty(): Boolean = ax == 0
    override fun contains(element: Axis): Boolean = has(element)
    override fun iterator(): Iterator<Axis> = Arrays.stream(Axis.VALUES).filter { it in this }.iterator()
    ...
    class Mutable : AxisSet {
        constructor(vararg axes: Axis) : super(*axes)
        internal constructor(v: Int) : super(v)

        fun withX(v: Boolean = true): Mutable = with(Axis.X, v)

        fun withY(v: Boolean = true): Mutable = with(Axis.Y, v)

        fun withZ(v: Boolean = true): Mutable = with(Axis.Z, v)

        fun with(axis: Axis, v: Boolean = true): Mutable {
            ax = set(ax, axis.ordinal, v)
            return this
        }

        fun withOr(another: AxisSet): Mutable {
            ax = ax or another.ax
            return this
        }

        fun withAnd(another: AxisSet): Mutable {
